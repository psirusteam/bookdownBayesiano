...
```{r, posNormalMVStan2, echo = FALSE, fig.cap="Distribuciones posteriores."}
p1 <- bayesplot::mcmc_areas(NormalMVfit, pars = c("theta"), prob = 0.95)
p2 <- bayesplot::mcmc_areas(NormalMVfit, pars = c("sigma"), prob = 0.95)
library(patchwork)
p1 / p2
```
...
```{r, plotsNormalMV, echo = FALSE, fig.cap="Convergencia de las distribuciones posteriores y diagramas de la función de autocorrelación en las cadenas.", fig.height=6, fig.width=8}
library(dplyr)
posteriores <- as.mcmc(
  data.frame(theta = theta.pos,
             sigma2 = sigma2.pos))

p1 <- bayesplot::mcmc_trace(posteriores)
p2 <- bayesplot::mcmc_acf_bar(posteriores)
...
Podemos ver que se espera que el promedio de las pruebas en 5 nuevos pacientes es de `r mean(y.bar)`, con un intervalo de credibilidad del 95% que es mucho más ancho que el de $\theta$, pues naturalmente $\bar{Y}$ tiene mayor incertidumbre que los parámetros del modelo; además, el tamaño de nuevos datos es de cinco, el cual es pequeño y hace que el pronóstico para $\bar{Y}^*$ no sea muy preciso. 
...
```{proposition}
La distribución conjunta previa de los parámetros $\theta$ y $\sigma^2$ está dada por
\begin{equation*}
...
```{proposition}
La distribución posterior de $\theta$ condicional a $\sigma^2,\mathbf{Y}$ está dada por
\begin{equation*}
...
```{proof}
Acudiendo a la distribución posterior conjunta dada en \@ref(eq:desarro1), tenemos que
\begin{align*}
...
```{proof}
De la distribución posterior conjunta \@ref(eq:desarro1), e integrando con respecto a $\theta$, se tiene que
...
```{proposition, PosThetaTnoestandar}
...
```{proposition, PosThetaNoInformativa}
La distribución posterior del parámetro $\theta$ sigue una distribución $t$ no estandarizada con $n-1$ grados de libertad, parámetro de localización $\bar{Y}$ y parámetro de escala $\frac{S^2}{n}$; esto es, 
...
```{proposition, PosSigma2NoInformativa}
...
```{r, PredictivaYprioriNoninformativa, fig.cap="10 mil valores simulados de $\\tilde{Y}$ y la función de densidad de la distribución predictiva de $\\tilde{Y}$ con parámetros no informativos."}
library("MCMCpack")
nsim <- 10000
y.tilde <- theta.pos <- c()
n <- 20
...
```{r, predictivaTsimu, fig.cap="10 mil valores simulados de $\\tilde{Y}$ y la función de densidad de la distribución predictiva de $\\tilde{Y}$ con parámetros informativos."}
library(MCMCpack)
nsim <- 10000
...
```{proposition}
La distribución posterior del vector $\btheta$ sigue una distribución normal multivariante
\begin{equation*}
...
```{r, posNormalMultiStan, fig.cap="Distribución posterior."}
...
```{r, posNormalDifMultiStan, fig.cap="Distribución posterior de los parámetros transformados (diferencia de las medias teóricas)."}
...
```{r, posNormalMVCov1, echo = FALSE, fig.cap="Distribuciones posteriores para el vector de medias y la diferencia de medias."}
bayesplot::mcmc_areas(NormalMultMediaCovfit, 
      pars = c("theta[1]", "theta[2]", "diftheta"), 
      prob = 0.95)
```
```{r, posNormalMVCov2, echo = FALSE, fig.cap="Distribuciones posteriores para los elementos de la matriz de covarinzas."}
bayesplot::mcmc_areas(NormalMultMediaCovfit, 
      pars = c("Sigma[1,1]", "Sigma[1,2]", "Sigma[2,2]"), 
      prob = 0.95)
```
...
```{r, echo = FALSE, fig.cap="Convergencia de las distribuciones posteriores y diagramas de la función de autocorrelación en las cadenas.", fig.height=6, fig.width=8}
library(dplyr)
posteriores <- as.mcmc(
  data.frame(theta1 = theta.pos[, 1],
             theta2 = theta.pos[, 2],
             Sigma11 = Sigma.pos[, 1, 1],
             Sigma22 = Sigma.pos[, 2, 2]))
...
```{r, Natalidad, echo=FALSE}
# Datos muestrales
data.frame(
  Natalidad = c(20.7, 46.6, 28.6, 23.4, 27.4, 
           32.9, 29, 34.8, 32.9,18,27.5), 
  Mortalidad = c(8.4, 18, 7.9, 5.8, 6.1, 7.4,
           23.2, 6.6, 8.3, 9.6, 4.4),
  MortalidadInfantil = c(25.7, 111, 63, 17.1, 40, 63,
           43, 42, 109.9, 21.9, 23.3)) %>%
  knitr::kable(caption = "Tasa de natalidad, tasa de mortalidad, mortalidad infantil en algunos países.")
```
...
En el anterior análisis, vemos que el intervalo de credibilidad para $\theta_1-2\theta_2$ contiene solo valores positivos, lo cual es un indicio de que la variable $\theta_1-2\theta_2$ tenga la mayor parte de la función de densidad ubicada en el eje positivo. De hecho podemos indagar por $Pr(\theta_1-2\theta_2>0)$, la cual se puede calcular de la distribución $t_{19}(10.8, 5.054)$ encontrada anteriormente. Esta probabilidad es `1 - pt((0 - 10.8)/sqrt(5.054), 19)` dando como resultado `r 1 - pt((0 - 10.8)/sqrt(5.054), 19)`; de donde se muestra una fuerte evidencia de que la tasa de natalidad es superior a dos veces la tasa de mortalidad.
...
```{example, EjeStudent3}
Retomamos los datos del efecto de aumento en horas de sueño de dos medicamentos soporíferos utilizados en los ejemplos \@ref(exm:EjeStudent) y \@ref(exm:EjeStudent2). Los siguientes códigos en `R` ilustran el procedimiento computacional para obtener valores de la distribución posterior conjunta de $\btheta$ y $\bSigma$.
```
...
```{r, multinom1, echo = FALSE, fig.cap="Distribuciones posteriores para el vector de probabilidades de interés."}
bayesplot::mcmc_areas(Multinomfit, 
...
```{r, multinom2, echo = FALSE, fig.cap="Distribuciones posteriores predictivas."}
bayesplot::mcmc_areas(Multinomfit, 
      pars = c("ypred[1]", "ypred[2]", 
               "ypred[3]", "ypred[4]"), 
      prob = 0.95)
```

```{r, multinom3, echo = FALSE, fig.cap="Distribuciones posteriores predictivas."}
bayesplot::mcmc_areas(Multinomfit, 
      pars = c("deltapred"), 
      prob = 0.95)
```
