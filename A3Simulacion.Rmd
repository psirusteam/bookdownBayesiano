...
```{example}
El método de la grilla también puede utilizarse para simular observaciones de una distribución biparamétrica, univariada y continua. El siguiente código computacional permite utilizar el método de la grilla para simular mil valores provenientes de una distribución Gamma con parámetros $\theta = 4$, $\beta = 2$.
```
...
```{example}
Se puede implementar el muestreador de Gibbs para generar una secuencia de observaciones con densidad
conjunta  
...
```{r, cache=TRUE, fig.cap="Generación de valores para una distribución normal bivariada."}
...
```{example}
Un problema común es el de descartar los primeros valores, puesto que el algoritmo puede demorar en obtener convergencia;
...
```{r, cache=TRUE, fig.cap="Convergencia de la media y varianza usando el muestreador de Gibbs."}
...
```{r, cache=TRUE, fig.cap="Cadenas generadas desde el muestreador de Gibbs."}
par(mfcol = c(1, 2))
plot(mc1.vals[, 1], type = 'l', ylab = 'mu')
plot(mc1.vals[, 2], type = 'l', ylab = 'sigma^2')
```

```{r, cache=TRUE, fig.cap="Densidades posteriores generadas con el muestreador de Gibbs."}
par(mfcol = c(1, 2))
...
```{proposition}
El muestreador de Gibbs es equivalente al algoritmo de Metropolis-Hastings, con la
...
colMeans(mc2)
```
```{r, fig.cap="Autocorrelación y autocorrelación parcial para las cadenas simuladas del algoritmo MH.", cache=TRUE}
par(mfrow=c(2,2))
pacf(mc2[, 1], 100)
pacf(mc2[, 2], 100)
acf(mc2[, 1], 100)
